...
This is a working test but it requires running cassandra node on localhost:9042.
We don't have embedded cassandra for test purposes so the test is commented to not fail the build
